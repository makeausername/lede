...
server_table="$root/luasrc/view/shadowsocksr/server_table.htm"
...
reject_text 'Node order can be dragged with the mouse and takes effect immediately.' "$servers"
require_text 'display: none !important;' "$server_table"
test "$(grep -Fc '<% render_pager() %>' "$server_table")" -eq 2
require_text 'id="cbi-server-page"' "$server_table"
require_text 'id="cbi-server-page-size"' "$server_table"
...
require_text 'if current == "nil" or current == "" then' "$runtime"
require_text 'return nil, false, "disabled"' "$runtime"
require_text 'return nil, true, "disabled-missing"' "$runtime"
reject_text 'cursor:get_first(CONFIG, SERVER_TYPE)' "$runtime"
reject_text 'cursor:set(CONFIG, global_sid, "global_server", first)' "$runtime"
